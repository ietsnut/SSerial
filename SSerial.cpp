...
{
  //  Serial.println("n-speed");
  //  Serial.print("rx:");
  //  Serial.println(_rxDisabled);
  //  Serial.print("tx:");
  //  Serial.println(_txDisabled);
  ...
  for (unsigned i = 0; i < sizeof(table) / sizeof(table[0]); ++i)
  {
    ...
    if (baud == speed)
    {
      if (!_rxDisabled) {
        ...
        //        Serial.println("rx speed");
      }
      // tx delay is needed for both rx and tx
      _tx_delay = pgm_read_word(&table[i].tx_delay);
      //      Serial.println("Tx speed");
      //      Serial.println("found table");
      break;
    }
  }
  ...
  if (_rx_delay_stopbit)
  {
    if (digitalPinToPCICR(_receivePin))
    {
      ...
      *digitalPinToPCMSK(_receivePin) |= _BV(digitalPinToPCMSKbit(_receivePin));
      //      Serial.println("set rx delay");
    }
    tunedDelay(_tx_delay); // if we were low this establishes the end
    //    Serial.println("set Tx delay");
  }
  ...
  // If tx is disabled, then delay is reset
  if (_txDisabled) {
    _tx_delay = 0;
  }

  // If rx pin is used, start listen port
  if (!_rxDisabled) {
    listen();
  }
}
